...
from battinfo.api import build_cell_type_library_rdf, query_library_cell_types, save_library_cell_type
...
def test_alpha_detailed_descriptor_matrix_validates_builds_and_queries(tmp_path: Path) -> None:
    ...
    coin_rows = query_library_cell_types(directory=library_root, format="coin")
    assert len(coin_rows) == 1
    assert coin_rows[0]["construction"]["assembly_type"] == "stacked"

    cylindrical_rows = query_library_cell_types(directory=library_root, format="cylindrical")
    assert len(cylindrical_rows) == 1
    assert cylindrical_rows[0]["construction"]["assembly_type"] == "wound"

    pouch_rows = query_library_cell_types(directory=library_root, format="pouch")
