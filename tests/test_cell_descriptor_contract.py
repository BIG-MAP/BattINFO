...
def test_battery_descriptor_examples_validate_against_normative_schema() -> None:
    ...
    assert example_paths, f"No cell-descriptor examples found in {examples_dir}"
...
def test_battery_descriptor_schema_files_are_synced_between_assets_and_package() -> None:
    ...
    tracked = [assets_root / "cell-descriptor.schema.json"]
...
def test_battery_descriptor_examples_are_synced_between_assets_and_package() -> None:
    assets_root = ROOT / "examples" / "cell-descriptors"
    package_root = ROOT / "src" / "battinfo" / "data" / "examples" / "cell-descriptors"

    tracked = sorted(assets_root.glob("*.example.json"))
    assert tracked, f"No cell-descriptor examples found in {assets_root}"
