...
class AuthoringWorkspace:
    ...
    def commands(self) -> None:
        ...
        groups = [
            ...
            ("Discover", ["search('samsung 50e')", "template('cell-spec', ...)", "bdf_columns()"]),
            ("Build",    ["convert()", "convert_csv(path, hints=...)", "load(src)",
                          "add('cell-instance', spec=spec, serial_numbers=[...])",
                          "add('test', kind='cycling', datasets='bdf/*.csv')"]),
    ...
    def quickstart(self) -> None:
        ...
        print(
            ...
            'ws.add("cell-instance", spec=spec, serial_numbers=["S1", "S2", "S3"])\n'
            "\n"
            "# 5. Attach tests to cells (files matched to cells by name)\n"
            'ws.add("test", kind="cycling", datasets="bdf/*.bdf.csv")\n'
            "\n"
    ...
    def save(self, validation_policy: str = "strict") -> dict:
        ...
        for obj in self._ws.cell_types:
    ...
    def _add_tests(
        ...
        # Validate the test kind early with a friendly, enumerated error.
        if kind:
            kind = _normalize_test_kind(kind)
        elif spec is None and protocol is None:
            raise ValueError(
                "ws.add('test', ...) requires kind=... (or a spec/protocol that "
                "defines it). " + _test_kind_hint()
            )
        ...
        # T2.2 — auto-reload cells saved in a previous session, so cross-session
        # `ws.add("test", ...)` cannot silently create tests with no cell linkage.
        ...
        if not self._cells_by_short_id:
            raise RuntimeError(
                "No cell instances registered, so dataset files cannot be matched.\n"
                "  Add them first, e.g.:\n"
                "      ws.add('cell-instance', spec=spec, serial_numbers=['S1', 'S2'])\n"
                "  If you saved cells in a previous session: ws.reload_cells()"
            )
        ...
        for f in matched_files:
            # Match by the 6-char short ID embedded in the filename, OR by the
            # full stem as a serial number (e.g. files named "TRFFC00174.csv").
            ...
            if test_spec is not None:
                if isinstance(test_spec, str):
                    ...
                    if spec_kind is not None and not file_kind:
                        file_kind = spec_kind.value if hasattr(spec_kind, "value") else str(spec_kind)
            if not file_kind:
                raise ValueError(
                    "Could not determine the test kind. Pass kind=... to "
                    "ws.add('test', ...). " + _test_kind_hint()
                )
            file_kind = _normalize_test_kind(file_kind)
            protocol_name = protocol_name or file_kind.replace("_", " ")
            ...
            print(f"  matched   {f.name}  ->  {cell.serial_number}  (by {how})")
            tests.append(test)

        if unmatched:
            known = sorted({
                c.serial_number for c in self._cells_by_short_id.values()
                if getattr(c, "serial_number", None)
            })
            print(f"\n  {len(unmatched)} file(s) had no matching cell instance:")
            for f in unmatched:
                print(f"    {f.name}")
            stems = ", ".join(repr(f.stem) for f in unmatched[:5])
            print("  Register the missing cells, then re-run, e.g.:")
            print(f"      ws.add('cell-instance', spec=spec, serial_numbers=[{stems}])")
            if known:
