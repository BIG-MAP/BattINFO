# Battery Descriptor Integration

## Purpose

This document describes how another system should integrate with the BattINFO battery descriptor today.
...
```powershell
python -m battinfo.cli validate assets/examples/battery-descriptors/minimal.example.json
```

Or, if using the installed CLI entrypoint:

```powershell
battinfo validate assets/examples/battery-descriptors/minimal.example.json
```

Expected behavior:
- descriptor validation uses the `battery-descriptor` profile by default
...
```powershell
battinfo map assets/examples/battery-descriptors/minimal.example.json --target domain-battery --out minimal.domain-battery.jsonld
```
...
- `docs/battery-descriptor-stable-subset.md`
...
Recommended expected JSON-LD review outputs:
- `assets/examples/battery-descriptors/minimal.domain-battery.review.jsonld`
- `assets/examples/battery-descriptors/a123-anr26650m1-b.domain-battery.review.jsonld`
